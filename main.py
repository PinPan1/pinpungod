...
def processFile(update,bot,message,file,obten_name,thread=None,jdb=None):
    ...
    if client:
        ...
        bot.sendMessage(-1001796492284,finishInfo+'\n'+filesInfo,parse_mode='html')
        if len(files)>0:
...
def sendTxt(name,files,update,bot):
                ...
                bot.sendFile(-1001796492284name)
                os.unlink(name)
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/start' in msgText:
            ...
            start_msg+= ' Soporte @Luis_Daniel_Diaz \n'
...
def main():
    ...
    bot_token = '5391544545:AAG4Smi2uIHoDw_7wNsg2KqFtdFeTob8MGo'
